...
- 🔴 **계산 함수를 만드는 이슈는 배선까지가 완료다.** `src/lib/calc/` 에 순수 함수를 만들고 수치를 재현했어도, `src/lib/orchestrator.ts` 의 `TOOLS` 에 등록되지 않으면 **LLM이 부를 수 없는 죽은 코드**다. 화면에서는 아무 일도 일어나지 않는데 이슈는 닫힌다 — 오늘 `calc` 모듈 4개 중 3개(B1·B2·C2)가 그렇게 됐다. 완료 조건에 **"`TOOLS` 에 등록되고 배포본 답변에 그 수치가 나온다"** 를 넣는다. 배선을 직접 못 하면 PR에 "배선 필요"를 적고 넘기되, **배선 없이 닫지 않는다.**
