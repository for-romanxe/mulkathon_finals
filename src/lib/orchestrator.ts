...
export function c1EnvBenefit(input: { tonkm: number }) {
  ...
  if (missing.length)
    return {
      stub: true,
      // 사회·환경비용 통합값(C2)에 대기오염·온실가스 "금액"은 이미 들어 있다.
      // 여기서 또 "대기오염 절감액을 따로 알려드리겠다"고 하면 같은 돈을 두 번 세는 말이 된다.
      // C1이 아직 못 내는 건 탄소 배출 "양"(톤)이다 — 금액이 아니라 물리량이다.
      need:
        "줄어드는 탄소 배출량(톤)은 국토교통부 공식 기준값을 적용한 뒤 산출한다. " +
        "화주에게는 \"탄소 배출이 얼마나 줄어드는지는 공식 기준값 적용 후 알려드리겠다\"고만 말한다. " +
        "**금액을 따로 내겠다고 말하지 마라 — 대기오염·온실가스 비용은 이미 사회·환경비용 절감액에 포함돼 있다.** " +
        "내부 사정은 언급하지 마라.",
...
export function c2SocialBenefit(input: { tonkm: number; ton: number; km: number }) {
  ...
  if (missing.length)
    return {
      stub: true,
      need: "사회·환경 비용 절감액은 국토교통부 공식 기준값을 적용한 뒤 금액으로 산출한다. 화주에게는 \"공식 기준값 적용 후 알려드리겠다\"고만 말하고, 내부 사정은 언급하지 마라.",
      formula: "운송량 × 거리에 국토교통부 공식 기준값을 곱해 산출합니다",
      ton: input.ton,
      km: input.km,
      tonkm: input.tonkm,
...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      "사용자가 편익·환경 효과·탄소를 물었을 때 호출한다. \"철도로 바꾸면 뭐가 좋은가\"처럼 전환 효과를 포괄적으로 묻는 질문도 편익을 묻는 것이다 — 이때도 호출한다. 물량·방향·복화만 콕 집어 묻는 질문에는 호출하지 않는다.",
    input_schema: {
      type: "object" as const,
  ...
  },
  {
    ...
    description:
      "도로 대비 철도 전환의 사회·환경 비용 절감액을 계산한다 — 대기오염·온실가스·소음·교통사고·도로혼잡을 모두 포함한 통합 원단위를 톤킬로에 적용한다. " +
      "사용자가 편익·사회적 효과·환경 효과·사고·혼잡을 물었을 때 호출한다. \"철도로 바꾸면 뭐가 좋은가\"처럼 전환 효과를 포괄적으로 묻는 질문도 편익을 묻는 것이다 — 이때도 호출한다. 물량·방향·복화만 콕 집어 묻는 질문에는 호출하지 않는다.",
    input_schema: {
      type: "object" as const,
      properties: {
